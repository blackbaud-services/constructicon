...
describe('Grid', () => {
  it('should render a grid container', () => {
    ...
    const rule = utils.findRule(css(), container.props().className)
  ...
  it('should allow us to set the spacing of the grid', () => {
    ...
    const rule = utils.findRule(css(), container.prop('classNames').root)
  ...
  it('should allow us to set breakpoints on a column', () => {
    ...
    const rule = utils.findRule(css(), column.prop('classNames').root)
    const ruleSm = utils.findRule(css(), `${column.prop('classNames').root}${mediaQuery('sm')}`)
    const ruleMd = utils.findRule(css(), `${column.prop('classNames').root}${mediaQuery('md')}`)
