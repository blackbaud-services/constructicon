...
export const configureMediaQueries = (breakpoints = []) => {
  ...
  breakpoints.map(breakpoint =>
    createRule({ [mediaQuery(breakpoint)]: { display: 'block' } })
  )
  reverse(breakpoints).map(breakpoint =>
    createRule({ [mediaQuery(breakpoint, 'max-width')]: { display: 'block' } })
  )
}

if (typeof window !== 'undefined') {
  const serverRenderedCSS = document.querySelectorAll(
    'style[data-fela-rehydration]'
  )
