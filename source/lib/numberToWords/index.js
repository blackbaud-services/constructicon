...
const sizes = (arrayLength) => {
  switch (arrayLength) {
    ...
    case 3: return ['million', 'thousand', '']
  }
}

const groupHasValue = (group) => (
  group.some(number => number !== '0')
)
...
const convert = (groupsOfHundreds) => (
  groupsOfHundreds
    .map((group, i) => {
      const isLast = (
        groupsOfHundreds.length > 1 &&
        groupsOfHundreds.length - 1 === i
      )
      return groupHasValue(group)
        ? `${convertHundreds(group, isLast)} ${sizes(groupsOfHundreds.length)[i]}`
        : ''
    })
    .join(' ')
    .replace(/\s{2,}/g, ' ')
...
const convertHundreds = (group, isLast) => {
  ...
  const hundredsWord = hundredsInt > 0
    ? `${words.ones[hundredsInt]} hundred`
    : ''
  const and = (hundredsInt || isLast) && tensInt
    ? 'and'
    : ''
